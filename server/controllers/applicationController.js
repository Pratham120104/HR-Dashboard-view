...
import Application from "../models/Application.js"; // ⬅️ NEW: Mongoose model
  ...
  try {
    ...
    for (const r of required) {
      if (!r.field || String(r.field).trim() === "") {
        return res
          .status(400)
          .json({ success: false, message: `${r.name} is required.` });
      }
    }
    if (!resumePath) {
      return res
    ...
    const hrMailOptions = {
      ...
      subject: `📩 New Job Application – ${jobTitle || "Untitled Job"} (ID: ${
        jobId || "N/A"
      })`,
      html: `
        ...
        <blockquote style="border-left:4px solid #ccc;padding-left:10px;white-space:pre-wrap;">
          ${comments}
        </blockquote>
    ...
    const applicantMailOptions = {
      ...
      html: `
        ...
        <p>Hi ${fullName},</p>
        <p>We have successfully received your application for the <b>${
          jobTitle || "position"
        }</b>.</p>
        <ul>
    ...
    let appRecord;
    try {
      appRecord = await Application.create({
        jobId: jobId || null,
        jobTitle: strip(jobTitle) || "Untitled role",
        fullName: strip(fullName),
        email: strip(email),
        phone: strip(phone),
        comments: strip(comments),
        resumePath, // stored so HR dashboard can build a download link
      });
      console.log("💾 Application stored in DB:", appRecord._id);
    } catch (dbErr) {
      console.error("❌ Failed to store application in DB:", dbErr);
      return res.status(500).json({
        success: false,
        message: "Application email sent but failed to store in database.",
      });
    }

    // ---- 5) (Optional) Cleanup: you probably DON'T want this now
    // If you keep this, resumes will NOT be downloadable later.
    // For persistent HR downloads, comment this out.
    /*
    setTimeout(() => {
      try {
 ...
 * Optional: ?jobId=<id> to filter by job.
 */
